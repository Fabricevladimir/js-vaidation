...
describe("validate", () => {
  ...
  test("should throw error when value is neither an object nor a string", () => {
    ...
    expect(() => validate(invalidValue, DEFAULT_SCHEMA)).toThrow(
  ...
  const validationResultTests = [
    ...
    {
      schema: new Schema().hasPattern(DEFAULT_VALUE).isRequired(),
      ruleName: "pattern",
      validValue: DEFAULT_VALUE,
      invalidValue: "S",
      validationError: [Messages.PATTERN],
    },
    {
...
describe("Validate Form", () => {
  ...
  schemaTests.map((fixture) => {
    ...
    test(`should throw error when ${description}`, () => {
      expect(() => validate(DEFAULT_FORM, schema)).toThrow(
        Errors.FORM_SCHEMA_MISMATCH
      );
    });
  });
